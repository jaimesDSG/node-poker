...
function checkForEndOfRound(table,oldRound) {
    var numPlayer = 0;
...
function checkForWinner(table) {
    ...
    table.game.winners.push(winners);
    ...
    for (i = 0; i < winners.length; i += 1) {
        table.players[winners[i]].chips += Math.round(prize / winners.length) ;
...
function checkForBankrupt(table) {
    ...
    for (i = 0; i < table.players.length; i += 1) {
        if (table.players[i].chips === 0) {
            table.game.bankrupt.push(table.players[i].playerName)
        }
    }
}
...
function rankHandInt(hand) {
    ...
    //Flush
    if (rank === 0) {
        if (suits.indexOf('CCCCC') > -1 || suits.indexOf('DDDDD') > -1 || suits.indexOf('HHHHH') > -1 || suits.indexOf('SSSSS') > -1) {rank = 123 + rankKickers(ranks, 5); }
...
function progress(table) {
    if (table.game) {
        if (checkForEndOfRound(table,true) === true) {
            result = gameProcessing(table);
            return result;
        } else {
            return {status: 0, result: 'Next Player'};
        }
    }
}

function gameProcessing(table) {
    var i, j, cards, hand;
    ...
    if (table.game.roundName === 'River') {
        ...
        console.log("Winner",table.game.winners,table.game.winners[0][0])
        return {status: 2, result: 'End of Round', data: {winners: table.game.winners[0], hand: table.players[table.game.winners[0][0]].hand}}
        ...
        if (checkForEndOfRound(table,false) === true) {
            return gameProcessing(table);
        } else {
            return {status: 1, result: 'End of Turn'};
        }
        ...
        if (checkForEndOfRound(table,false) === true) {
            ...
            return {status: 1, result: 'End of Flop'};
        }
        ...
        if (checkForEndOfRound(table,false) === true) {
            ...
            return {status: 1, result: 'End of Deal'};
        }
    }
}
...
Table.prototype.getMaxBets = function() {
    return getMaxBet(this.game.bets);
};

Table.prototype.cleanTable = function() {
    var i;
    for (i = 0; i < this.players.length; i += 1) {
...
Player.prototype.Check = function() {
    ...
    for (v = 0; v < this.table.game.bets.length; v += 1) {
        if (this.table.game.bets[v] !== getMaxBet(this.table.game.bets)) {
            checkAllow = false;
        }
    }
    if (checkAllow) {
        ...
        return progress(this.table);
    } else {
        return {status: 10, result: 'Check not allowed, replay please'};
    }
};
...
Player.prototype.Fold = function() {
    ...
    for (i = 0; i < this.table.players.length; i += 1) {
        if (this === this.table.players[i]) {
            bet = parseInt(this.table.game.bets[i]);
            this.table.game.bets[i] = 0;
